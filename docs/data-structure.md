...
突围地图的 41 个地块和 81 条无向边均显式写入 `maps/breakout.json`。地图使用逻辑半行坐标；显示坐标为第 6 列的六个格子整体下移半行，依次位于 `2.5` 至 `7.5` 行。第 3 列的两个堡垒仍位于 `2.5`、`3.5` 行，第 1 行的两个目标格仍使用 `7.5`、`8.5` 半列坐标。这些错位坐标会直接写入格子和连接表，占位符 `0` 不生成地块或连接。
...
当前突围场景默认提供 12 点进攻增援。进攻方部署上限为 12 步兵、1 反坦克炮和 0 机枪车；防守方部署上限为 4 步兵、0 反坦克炮和 2 机枪车。第 28 回合完整结束后，进攻方仍未达成目标区域胜利时由防守方获胜。

## 人机配置

`ai-profiles` 不绑定战斗 ID 或地图 ID。它依赖可复用的地形类型和单位类型，并保存部署、射击、移动、局面估值、难度与搜索限制。战斗入口通过资源引用选择机器人配置，因此多个战斗可以显式共享同一算法。当前经典模式和突围都引用 `ai-profiles/light-search.json`。部署候选会同时计入棋盘单位和本轮规划中尚未结算的单位，因此一个兵种达到上限后仍会继续检查其他可部署兵种。顺序回合开启“允许行动后部署”时，机器人会在每次行动后重新检查部署条件，以使用行动腾出的部署格；关闭该规则时，机器人只在本方单位开始行动前部署。

`light-search` 使用有界递归束搜索。简单难度前瞻 3 层，普通难度前瞻 4 层，困难难度前瞻 6 层。每个难度分别限制根候选数、后续分支数、总节点数和单次搜索时间；服务器达到任一上限就立即使用已经得到的最佳结果。当前配置分别限制为 40、192、384 个节点和 8、20、45 毫秒。搜索仍然是截断的有限束搜索，不会展开完整动作树；深度增加通过节点和时间上限约束，适合在低性能服务器上渐进提高强度。

局面评估综合单位价值、受伤状态、地形收益、受威胁程度、相邻可通行格、场上兵力、进攻方到目标的图距离、防守方目标覆盖和回合上限临近时的紧迫度。动作评估还考虑立即击杀、无效射击、射击后的潜在位置、推进、封堵和机动空间。所有数值权重都保存在 AI 配置中；算法只使用服务端规则引擎生成的合法部署和行动，并按地图显式连接计算距离，因此同一配置可处理经典棋盘和突围的不规则地图。
