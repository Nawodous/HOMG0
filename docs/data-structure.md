...
突围地图的 41 个地块和 71 条无向边均显式写入 `maps/breakout.json`。新版地图使用 `2.5`、`3.5` 两个逻辑半行，以及第 1 行的 `7.5`、`8.5` 两个半列坐标；这些错位坐标会直接写入格子和连接表。占位符 `0` 不生成地块或连接。
...
经典模式的值为 `null`。突围模式的特殊规则只使用当前白名单能力：地图区域、双方部署区、兵种阵营与数量限制、默认增援和进攻目标区域。突围地图中编号 `5` 对应的格子同时属于防守方部署区和进攻方目标区，两个区域在序列化数据中显式包含这些相同坐标。特殊规则中的资源依赖、区域坐标和单位 ID 均在启动时校验。

突围的 `victory.roundLimit` 将回合上限、到期胜方和胜利原因序列化在场景规则中。回合上限只在一个完整回合结算结束后检查；目标区域胜利和单位消灭判定优先于回合上限，因此同时回合制在最后一回合的移动结算中仍可达成进攻方胜利。

当前突围场景默认提供 12 点进攻增援。进攻方部署上限为 12 步兵、1 反坦克炮和 0 机枪车；防守方部署上限为 5 步兵、0 反坦克炮和 2 机枪车。第 28 回合完整结束后，进攻方仍未达成目标区域胜利时由防守方获胜。
